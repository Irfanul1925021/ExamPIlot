...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($newPassword === $confirmedPassword) {
        ...
        $updatePasswordSql = "UPDATE individual_user SET password = '$hashedNewPassword' WHERE email = '$userEmail'";
        // $updatePasswordSql = "UPDATE individual_user SET joining_id = '100' WHERE email = '$userEmail'";
        echo 'success';
        if ($conn->query($updatePasswordSql)) {
            echo "<script>alert('$userEmail');alert('$hashedNewPassword') ;window.location.href = 'M-1_P-2.php';</script>";
        } else {
